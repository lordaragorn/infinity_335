        ...
        case NA_NPC_GUARD_15:
            {
                ...
                else if (CreatureData const * cd = sObjectMgr.GetCreatureData(itr->second))
                {
                    if(!cd->is_dead)
                        ++cnt;
                }
...
void OPvPCapturePointNA::SpawnNPCsForTeam(Team team)
{
...
void OPvPCapturePointNA::SpawnGOsForTeam(Team team)
{
...
OPvPCapturePoint(pvp), m_capturable(true), m_GuardsAlive(0),m_HalaaState(HALAA_N), m_WyvernStateSouth(0), m_WyvernStateNorth(0), m_WyvernStateWest(0),
